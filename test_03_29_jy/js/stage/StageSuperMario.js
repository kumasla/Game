    ...
    class StageSuperMario extends Phaser.Scene {
        constructor() {
            ...
            this.player;        
            this.stageTimer;
            this.stageDuration = 10; // 스테이지 지속 시간 (초)
            this.currentStage = 1;
            this.timerText;
            this.timer;
        ...
        preload() {
            this.load.spritesheet('tiles', 'assets/background/tiled/tile.png', { frameWidth: 156, frameHeight: 156, spacing: 0 });
            // 미사일 몬스터 관련 이미지
        ...
        create(data) {
            ...
            // 캐릭터 네임 보내기
            this.player = this.masterController.init('megaman');

            // image의 tile화 = frameWidth 나누듯이 나누면 됨, width와height는 개수
            const map = this.make.tilemap({ tileWidth: 156, tileHeight: 156, width: 10, height: 10 });
            ...
            // 일시정지 기능 / 버튼
            const pauseButton = this.add.text(10, 10, 'Pause', { fill: '#ffffff' });
            pauseButton.setInteractive();
            pauseButton.on('pointerdown', () => {
                this.scene.launch('PauseMenu', { player:this.player, bgm:this.bgm});
                this.scene.pause();
                this.bgm.pause();
            });
        ...
        showItemSelectionScene() {
            ...
            this.scene.launch('ItemSelectionScene', { player: this.player});
