class WeaponController{
    ...
    update(){
        ...
        for (let i = 0; i < this.weapons.length; i++) {
            ...
            if (closestMonster && closestDistance <= weapon.range) {
                ...
                switch(true){
                    case weapon.type == "sniper": case weapon.type == "rifle" : case weapon.type == "artillery" :
                    ...
                    case weapon.type=="sword":
                        this.attackWeapon(weapon, angle)
                        break;
    ...
    attackWeapon(weapon, angle) {
        if (this.scene.time.now - weapon.lastFired > (weapon.fireRate / this.scene.masterController.characterController.characterStatus.attackSpeed)) {
            ...
            const distance = 500; // 무기의 이동 거리 (임의로 설정)
            

            // 애니메이션 추가
            this.scene.tweens.add({
                targets: weapon,
                x: weapon.image.x + Math.cos(angle) * distance, // 무기가 앞으로 이동하는 x 좌표
                y: weapon.image.y + Math.sin(angle) * distance, // 무기가 앞으로 이동하는 y 좌표
                duration: 2000, // 애니메이션 지속 시간 (밀리초)
                onComplete: () => {
                    // 애니메이션 완료 후 처리할 내용
                    weapon.attack = false; // 공격 플래그를 false로 설정하여 다시 공격 가능 상태로 변경
    ...
    addWeapon(weaponName){
        ...
        for (let i = 0; i < this.weapons.length; i++) {
            ...
            if (!this.weapons[i].image) {
                ...
                //몬스터와 근접무기 충돌설정
                if(selectedWeapon.type == 'sword'){
                    this.addCollision(weaponObject);
                }
            }
        }
    ...
    addCollision(weaponObject) {
        ...
        // 이미지 객체에 물리 엔진 추가
        this.scene.physics.add.existing(weaponObject.image);
        ...
        this.scene.physics.add.overlap(
            ...
            monsterGroup,
            (weapon, monster) => {
                this.weaponsCollision(weapon, monster);
            },
            null,
            this
        );
