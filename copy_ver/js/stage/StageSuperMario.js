class StageSuperMario extends Phaser.Scene {
    ...
    create() {
        ...
        */
        //timer 로딩
        // 타이머 텍스트 객체 초기화
        this.timerText = this.add.text(this.cameras.main.centerX, 16, '', {
            fontSize: '32px',
            fill: '#ffffff'
        }).setOrigin(0.5, 0).setScrollFactor(0);

        // 타이머 시작
        this.timerManager.startTimer(this.stageDuration, this.nextStageSelection.bind(this));
    ...
    //추가함
    updateTimerDisplay() {
        const remainingSeconds = Math.floor(this.timerManager.getRemainingSeconds());
        this.timerText.setText(`남은 시간: ${remainingSeconds}`);
    }
    ...
    startNormalStage(){
        ...
        this.scene.launch('ItemSelectionScene', { player: this.player, characterStatus : characterStatus, weapons:weapons, masterController: this.masterController, settlement : this.settlement});
